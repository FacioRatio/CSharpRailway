...
{
    public class ResultTeeTaskABCDExtensionsTests
    {
        [Fact]
        public async Task Tee_Action_Success()
        {
            var sut = Task.FromResult(Result.Ok<(int, int, int, int)>((1, 2, 3, 4)));

            var i = 0;
            var result = await sut.Tee((a, b, c, d) => i = a * b * c * d);
        ...
        {
            var sut = Task.FromResult(Result.Fail<(int, int, int, int)>("fail"));
