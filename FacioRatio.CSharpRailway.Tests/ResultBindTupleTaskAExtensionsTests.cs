...
{
    public class ResultBindTupleTaskAExtensionsTests
    {
        [Fact]
        public async Task BindTuple_AResultB_Fails()
        {
            var sut = Task.FromResult(Result.Fail<string>("fail"));

            var result = await sut.BindTuple(s => Result.Ok(1));
        ...
        [Fact]
        public async Task BindTuple_AResultB_Succeeds()
        {
            var sut = Task.FromResult(Result.Ok("success"));
        ...
        [Fact]
        public async Task BindTuple_AB_Fails()
        {
            var sut = Task.FromResult(Result.Fail<string>("fail"));

            var result = await sut.BindTuple(s => 1);
        ...
        [Fact]
        public async Task BindTuple_AB_Succeeds()
        {
            var sut = Task.FromResult(Result.Ok("success"));
