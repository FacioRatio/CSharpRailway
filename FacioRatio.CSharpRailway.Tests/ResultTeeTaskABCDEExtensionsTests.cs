...
{
    public class ResultTeeTaskABCDEExtensionsTests
    {
        [Fact]
        public async Task Tee_Action_Success()
        {
            var sut = Task.FromResult(Result.Ok<(int, int, int, int, int)>((1, 2, 3, 4, 5)));

            var i = 0;
            var result = await sut.Tee((a, b, c, d, e) => i = a * b * c * d * e);
        ...
        {
            var sut = Task.FromResult(Result.Fail<(int, int, int, int, int)>("fail"));
